...
执行后 CLI 会输出登录地址和授权码，并进入"等待用户授权中..."轮询状态。拿到登录地址和授权码后，本轮只把它们发给用户。

当前 OpenClaw 消息会话不能稳定依赖后台登录进程在同一轮内继续推进。拿到登录地址后，必须回复用户完成授权后提醒；不要承诺本轮会自动继续创建、修改或删除。

6. 用户主动说"已授权""已登录"或类似表达后，立即执行 `contact user get` 检查登录态；检查通过后继续原操作，检查失败再说明登录未完成。
...
命令报错时按 `references/errors.md` 处理。
...
- 当前没有 `chat` / `im` 相关命令实现。
- 当前没有独立的组织/部门搜索、组织列表、组织详情命令。
- 当前没有独立的视频会议创建、会议链接、会议号创建命令。
